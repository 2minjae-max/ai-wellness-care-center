...
app.post("/api/health/analyze-prescription", upload.single("prescriptionImage"), async (req, res): Promise<void> => {
  ...
  if (!ai) {
    ...
    // [교육용 주석] 사용자가 업로드하는 실제 복약지도서 이미지와 결과를 100% 매칭하기 위해
    // 기존의 단순 더미(아모디핀, 리피토) 대신 이미지에 등장하는 실제 6대 약물의 정밀 정보를 기본 시뮬레이션 데이터로 탑재합니다.
    const simulatedResponse = {
      medications: [
        {
          name: "원투스테리드연질캡슐 (두타스테리드 0.5mg / 타다라필 5mg 복합제)",
          dosage: "1일 1회, 아침 식사 직후 복용 (일분)",
          efficacy: "전립선 비대증 치료 및 하부요로 증상 개선제",
          sideEffects: "남성 환자에게서 드물게 성기능 저하(리비도 감소, 사정 장애)가 생길 수 있으니 전문가와 상의하십시오. 여성과 소아는 약물 접촉을 피해야 합니다."
        },
        {
          name: "데스민정 0.1밀리그램 (데스모프레신아세트산염)",
          dosage: "1일 1회, 취침 전 복용 (식전 1시간~식후 2시간)",
          efficacy: "항이뇨제 (일차성 야뇨증 치료)",
          sideEffects: "이 약 복용 전 1시간부터 복용 후 8시간까지는 수분 섭취를 엄격히 제한하십시오. 과량 섭취 시 저나트륨혈증성 두통 및 구토가 유발될 수 있습니다."
        },
        {
          name: "유로파서방정 0.2밀리그램 (탐스로신염산염)",
          dosage: "1일 1회, 취침 전 복용 (특별한 지시가 없는 한)",
          efficacy: "전립선 비대증에 따른 배뇨장애 완화제",
          sideEffects: "일어설 때 혈압 저하로 인한 현기증(기립성 저혈압)이 일시적으로 생길 수 있으므로 자세를 바꿀 때 천천히 일어나는 것이 좋습니다."
        },
        {
          name: "발사르반정 80mg (발사르탄)",
          dosage: "1일 1회, 아침 식사 직후 복용",
          efficacy: "혈압 강하제 (안지오텐신II 수용체 차단제)",
          sideEffects: "증상이 호전되더라도 임의로 복용을 중단하거나 투여량을 줄이지 마십시오. 복용 초기 가벼운 두통이나 현기증이 관찰될 수 있습니다."
        },
        {
          name: "제미메트서방정 50/1000밀리그램 (제미글립틴/메트포르민 복합제)",
          dosage: "1일 1회, 아침 식사 직후 복용 (위장장애 예방)",
          efficacy: "당뇨병 치료제 (인슐린 비의존성 당뇨 조절제)",
          sideEffects: "반드시 식사와 함께 복용하여 소화기계 불편감을 최소화하십시오. 복용 중인 상태에서 음주할 경우 심각한 젖산산증 부작용이 생길 수 있으니 절대 금주하십시오."
        },
        {
          name: "에라빅스정 (클로피도그렐황산염)",
          dosage: "1일 1회, 아침 식사 직후 복용",
          efficacy: "항혈소판제 (혈전 생성 억제 및 뇌졸중/심근경색 예방)",
          sideEffects: "코피, 잇몸 출혈 및 생리과다 등의 출혈 증상이 멈추지 않을 경우 의사와 상담하십시오. 발치나 수술 전에는 사전 복약 중단이 필요합니다."
        }
      ],
      rawTextSummary: "제출된 복약안내 이미지에서 고혈압 치료제(발사르반정), 당뇨병 치료제(제미메트서방정), 전립선비대증약(원투스테리드, 유로파서방정), 항이뇨제(데스민정), 항혈소판제(에라빅스정) 등 총 6종의 복합 질환 약물이 확인되었습니다. 당뇨약 복용 시에는 반드시 금주를 하셔야 하며, 항이뇨제 복용 시간에 맞춘 엄격한 수분 제한과 항혈소판제 복용에 따른 출혈 주의 관리가 요망됩니다.",
      isSimulated: true
    };
