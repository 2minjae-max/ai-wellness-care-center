 ...
 * 역할:
 * 1. 한화손보 상품공시실(https://www.hwgeneralins.com/notice/ir/product-ing01.do) 웹크롤링
 * 2. 판매중인 '장기보험' 카테고리 필터링
 * 3. 핵심 3대 상품의 '상품요약서', '사업방법서', '약관' PDF 다운로드 및 리딩
 * 4. Gemini API를 활용한 PDF 구조화 지식 요약 추출
...
async function runBatch() {
  ...
  // 2. 대표 3대 상품 공시 매칭 진단
  console.log(`${logPrefix} 2단계: 핵심 주력 상품 매칭 진단 및 PDF 링크 수집 중...`);
  const productsToCollect = [
    { name: "한화 시그니처 여성 건강보험4.0[HOT]", type: "여성건강" },
    { name: "한화 시그니처 여성 건강보험4.0[새창][NEW]", type: "여성건강" },
    { name: "한화 더건강한 한아름종합보험 무배당[NEW]", type: "종합건강" },
    { name: "한화 실손의료보험(갱신형)", type: "실손의료" }
  ];

  console.log(`${logPrefix} 매칭 성공 상품 목록:`);
  ...
  const wikiData = {
    ...
    batchLog: "Success - Processed 4 representative Hanwha Insurance active long-term products.",
    products: {
